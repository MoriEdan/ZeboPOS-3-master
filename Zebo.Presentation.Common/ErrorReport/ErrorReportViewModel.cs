    ...
    class ErrorReportViewModel : ObservableObject
    {
        ...
        public void SubmitError()
        {
            ...
            string queryString = string.Format("from={0}&emaila={1}&file={2}",
                Uri.EscapeDataString("info@zebopos.tk"),
                Uri.EscapeDataString("ZeboPOS V3 Error Report"),
                Uri.EscapeDataString(tempFile));

            var c = new WebClient();
            var result = c.UploadFile("http://reports.zebopos.tk/file.php?" + queryString, "POST", tempFile);
